...
1. 小程序通过 `wx.login()` 获取临时登录凭证 code
2. 小程序将 code 发送到开发者服务器
3. 开发者服务器通过 code 向微信接口服务获取 openid、session_key 等信息
4. 开发者服务器自定义登录态，关联用户身份
5. 开发者服务器返回自定义登录态给小程序
6. 小程序保存登录态（token）用于后续业务请求身份校验

### 0.2 获取微信openid和session_key

- **接口路径**: `/api/wx/code2session`
- **请求方式**: POST
- **功能说明**: 通过wx.login()获取的临时code换取用户openid和session_key
- **请求参数**:

```json
{
  "code": "临时登录凭证"    // 通过wx.login()获取的临时登录凭证
}
```
...
```json
{
  ...
  "data": {
    ...
    "token": "JWT_TOKEN",         // 自定义登录态Token(如果已注册)
    "isNewUser": true,            // 是否新用户(true=新用户,false=老用户)
    "isRegistered": false         // 是否已完成注册(true=已注册,false=未注册)
  }
}
```
...
- **接口路径**: `/api/driver/silent-login`
- **请求方式**: POST
- **功能说明**: 已有用户信息的用户进行静默登录(无需用户操作)
- **请求参数**:

```json
{
  "driverId": "123456",    // 司机ID
  "code": "wx_login_code"  // 微信登录凭证(可选，用于刷新session_key)
}
```
...
- **接口路径**: `/api/wx/phone`
- **请求方式**: POST
- **功能说明**: 解密微信获取手机号接口返回的信息
- **请求参数**:

```json
{
  "code": "xxx"  // 手机号获取凭证(getPhoneNumber返回)
}
```
